...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for user in orm.UserProfile.objects.all():
            user.user_slug = slugify(user.user.username)
        
            slug_version = 1
            while (UserProfile.objects.filter(user_slug__iexact=user.user_slug).exists()):
                user.user_slug = "%s%s" % (slugify(user.user.username), str(slug_version))
                slug_version += 1
            user.save()
